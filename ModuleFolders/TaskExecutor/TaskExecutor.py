...
class TaskExecutor(Base):
    ...
    def task_manual_export(self, event: int, data: dict) -> None:
        ...
        # 使用项目独立目录
        project_output_path = self.cache_manager.get_project_output_directory(output_path)
        ...
        self.print("")
        self.info(f"翻译结果已成功保存至项目目录: {project_output_path}")
        self.print("")
    ...
    def task_start(self, event: int, data: dict) -> None:
        ...
        # 如果启用了多智能体模式，禁用传统翻译执行器
        # 传统翻译必须通过多智能体工作流执行
        if use_multi_agent and current_mode == TaskType.TRANSLATION:
            self.info("检测到多智能体模式，传统翻译执行器已禁用。翻译将通过Griptape工作流执行。")
            return None
    ...
    def translation_start_target(self, continue_status: bool) -> None:
        ...
        self.print("")
        self.info(f"翻译结果已保存至项目目录: {project_output_path}")
        self.print("")
    ...
    def polish_start_target(self, continue_status: bool) -> None:
        ...
        self.print("")
        self.info(f"润色结果已保存至项目目录: {project_output_path}")
        self.print("")
