...
class CacheManager(Base):
    ...
    def save_to_file(self) -> None:
        ...
        """
        # 为每个项目创建独立的子目录
        project_dir = self._get_project_directory(self.save_to_file_require_path)
            ...
            try:
                ...
                if self.project and self.project.stats_data:
                    ...
                    try:
                        ...
                        self.info(f"项目统计信息已保存到: {json_path}")
                    finally:
                ...
                # 在保存完成后，打印项目目录信息
                self.info(f"✅ 项目数据已保存到独立目录: {project_dir}")
            finally:
    ...
    def _get_project_directory(self, output_path: str) -> str:
        """
        获取项目的独立目录路径（内部方法）
        ...
        Returns:
            项目独立目录的完整路径
        """
        if not self.project or not self.project.project_name:
            # 如果没有项目名称，使用旧的路径结构
            return output_path
        
        # 清理项目名称
        safe_name = self._sanitize_project_name(self.project.project_name)
        
        # 返回项目独立目录
        return os.path.join(output_path, safe_name)
    
    def get_project_output_directory(self, base_output_path: str) -> str:
        """
        获取项目的独立输出目录路径（公共方法）
        ...
        Args:
            base_output_path: 基础输出根目录
            
        Returns:
            项目独立目录的完整路径
        """
        return self._get_project_directory(base_output_path)
    ...
    def load_from_file(self, output_path: str) -> None:
        """从文件加载数据，支持项目独立目录"""
        # 首先尝试从旧路径加载（兼容性）
        old_path = os.path.join(output_path, "cache", "AinieeCacheData.json")
        
        with self.file_lock:
            if os.path.isfile(old_path):
                # 加载旧项目
                self.project = self.read_from_file(old_path)
                self.info(f"从旧路径加载项目: {old_path}")
                # 加载后自动迁移到新路径
                if self.project and self.project.project_name:
                    new_project_dir = self._get_project_directory(output_path)
                    self.info(f"将项目迁移到新路径: {new_project_dir}")
                    self.save_to_file_require_path = output_path
                    self.save_to_file()  # 保存到新路径
            else:
                # 尝试从项目独立目录加载
                # 扫描所有项目子目录
                if os.path.isdir(output_path):
                    for item in os.listdir(output_path):
                        project_dir = os.path.join(output_path, item)
                        if os.path.isdir(project_dir):
                            cache_path = os.path.join(project_dir, "cache", "AinieeCacheData.json")
                            if os.path.isfile(cache_path):
                                self.project = self.read_from_file(cache_path)
                                self.info(f"从项目目录加载: {project_dir}")
                                # 只加载第一个找到的项目
                                break
