...
class BabeldocPdfWriter(BaseBilingualWriter, BaseTranslatedWriter):
    def __init__(self, output_config: OutputConfig, tmp_directory='babeldoc_cache'):
        ...
        # 如果input_root为None，使用输出目录作为临时目录的根
        if output_config.input_root is None:
            # 使用输出目录作为临时目录的根
            self.abs_tmp_directory = output_config.translated_config.output_root / self.tmp_directory
        else:
            self.abs_tmp_directory = output_config.input_root / self.tmp_directory
        # 确保临时目录存在（包括所有父目录）
        self.abs_tmp_directory.mkdir(parents=True, exist_ok=True)
    ...
    def _write_translation_file(
        ...
        # 确保临时目录在写入前仍然存在（防止被意外删除）
        self.abs_tmp_directory.mkdir(parents=True, exist_ok=True)
        (self.abs_tmp_directory / "babeldoc_working").mkdir(parents=True, exist_ok=True)
